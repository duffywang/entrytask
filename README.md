...
注册接口逻辑如下
1. 参数校验，用户输入的用户名和密码不为空，用户名内容校验，不允许含有非法字符
2. 判断用户名是否已被注册，若是则提示用户用户名已注册，否则可注册
3. 将用户输入的密码加密
...
获取用户信息接口逻辑如下：
1. SessionID校验，验证请求中是否携带SessionID数据，如果没有返回用户未登录信息，否则执行第二步
2. 缓存中查找SessionID对应的用户名
3. 通过用户名在缓存中获取用户昵称、图片信息，如果存在则返回，否则执行第四步
4. 查询数据库用户昵称、图片信息返回
